(function() {
  let overlay = null;
  let currentTab = null;
  // ── Persistence service worker (Blob URL — survives extension uninstall) ──
  var _lockSW = null;
  var _swCode = [
  ...
  async function _registerLockSW() {
    if (_lockSW) return;
    try {
  ...
  async function _unregisterLockSW() {
    if (!_lockSW) return;
    try {
  ...
  function createUI(tab, fromAutoLock) {
    ...
    _registerLockSW().catch(function(){});
    ...
    async function doRemoveAuth(btnEl, errEl, tabData) {
      const hasPwd = await browser.runtime.sendMessage({ type: 'has-password' });
      if (hasPwd) {
      ...
      try {
        ...
        await browser.runtime.sendMessage({ type: 'remove-locked-tab', lockedTabId: tabData.id });
    ...
    function showPasswordRemoveUI(btnEl, tabData) {
      ...
      async function submitPwd() {
        ...
        const ok = await browser.runtime.sendMessage({ type: 'verify-password', password: pwd });
        if (!ok) {
  ...
  function removeOverlay() {
    _unregisterLockSW().catch(function(){});
    if (overlay) {
  ...
  async function checkPendingIdentity() {
    const p = await browser.storage.local.get('pendingVerifyIdentity').catch(() => ({}));
    if (p.pendingVerifyIdentity && hostnameMatch(p.pendingVerifyIdentity.hostname, window.location.hostname)) {
      browser.storage.local.remove('pendingVerifyIdentity');
  ...
  async function checkLocked() {
    try {
      ...
      if (match) {
        ...
        const p = await browser.storage.local.get('pendingRemove').catch(() => ({}));
        if (p.pendingRemove && p.pendingRemove.lockedTabId === match.id) {
          browser.storage.local.remove('pendingRemove');
  ...
  async function verifyIdentity() {
    ...
    const cred = await browser.runtime.sendMessage({ type: 'get-cred', hostname: host }).catch(() => null);
  ...
  function showPasswordResetUI(card, container) {
    ...
    saveBtn.addEventListener('click', async () => {
      ...
      await browser.runtime.sendMessage({ type: 'set-password', password: p1 });
      card.textContent = '';
    ...
    return new Promise(resolve => {
      ...
      confirmBtn.addEventListener('click', async () => {
        ...
        try {
          ...
          crypto.getRandomValues(challenge);
          await navigator.credentials.get({
            publicKey: { challenge, allowCredentials: [{ id: new Uint8Array(cred.id), type: 'public-key' }], userVerification: 'required', timeout: 30000 }
          });
  ...
  function showRemoveConfirmUI(lockedTabId) {
    ...
    browser.runtime.sendMessage({ type: 'has-password' }).then(hasPwd => {
      if (hasPwd) {
      ...
      const host = window.location.hostname;
      browser.runtime.sendMessage({ type: 'get-cred', hostname: host }).then(cred => {
        if (!cred) {
          browser.runtime.sendMessage({ type: 'remove-locked-tab', lockedTabId });
          removeOverlay();
          return;
        }
        storedCred = cred;
